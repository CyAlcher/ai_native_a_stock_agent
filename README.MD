...
[任意-AI-都能用](#任意-ai-都能用-deepseek--kimi--豆包--chatgpt)
...
# 2. 一键装
bash install.sh          # 自动识别本地 AI 工具
# 或指定：bash install.sh claude / codex / gemini / cursor / all
...
- [ ] 增加「盘后复盘」自动模板，输入当日成交记录直接产出复盘
- [ ] 让 `/gu/index` 变成交互式问答向导，用户不必记住节点名
...
**ai_native_a_stock_agent** is a prompt library that covers the entire
retail-investor decision loop on the Chinese A-share market:
**11 trading stages × 83 high-frequency dilemmas**, each backed by a
structured analysis framework that plugs directly into Claude Code,
Codex CLI, Gemini CLI, or Cursor.

Drop a ticker, type your hesitation in plain Chinese — the agent routes
your question to the right framework (pre-entry / accumulation /
drawdown / stop-loss / profit-taking / review / cool-down) and returns
a structured answer with checklist, sizing tiers, and risk reminders.

> Topics suggested for GitHub repo settings:
> `a-share` `chinese-stock-market` `ai-agent` `claude-code` `codex-cli`
> `gemini-cli` `cursor` `prompt-library` `deepseek` `trading-psychology`
> `decision-framework`
